...
class LeadListCreateView(CreateAPIView,ListAPIView):
    ...
    def get_queryset(self):
        filter_text = self.request.query_params.get('search_text') # 'hello'
        
        qs = Lead.objects.all()
        
        if filter_text:
            search_value = filter_text.get('search_text')
            qs = qs.filter(Q(name_contains=search_value) | Q(emailcontains=search_value) | Q(mobile_contains=search_value))
